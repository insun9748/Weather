...
def generate_case_wrong_concepts(case: dict, wrong_texts: List[str]) -> List[Dict[str, str]]:
    ...
    system_prompt = (
        ...
        "마지막 줄에 이 사건에 실제로 나오는 개념 이름들을 화살표(→)로 연결한 한 줄 요약을 덧붙여라. "
        "'개념A', '개념B'같은 자리표시자 단어를 절대 그대로 쓰지 말고, "
        "반드시 실제 기상·기후 용어로 채워라. 예: '남서풍 강화 → 수증기 공급 증가 → 강수량 증가'처럼 "
        "이 사건 내용에 맞는 진짜 용어 3개를 순서대로 연결해라.)"
    )
